...
此STL项目的实现主要是参考了sgi-STL的源码,也增加了少许自己的理解和实现,你在学习过程中一定会发现许多不完整的地方,那是故意为之,stl中虽有许多容器,但是核心思想非常相似,像构造函数的参数,大多都是一摸一样的,只是实现根据不同的数据结构而有所不同,所以我只在前面的序列容器中实现,后面的关联容器中只实现必要的函数
